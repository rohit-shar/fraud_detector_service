  ...
  override def receive: Receive = {
    ...
    case OrderRequestReceiveCommand(orderNumber, order) =>
      ...
      if (!fraudDetectionDatabaseService.isOrderAlreadyExists(orderNumber)) {
        ...
        var orderEmailSent = orderResponseDecisionActor ? DuplicateOrder(order)
        orderEmailSent onComplete {
          case Success(response: OrderEmailSent) => {
            if (response.status == 202) {
              log.info("EMAIL SENT SUCCESSFULLY FOR DUPLICATE ORDER")
              log.info("RESPONSE STATUS CODE IS : ", response.status)
            }
            else {
              log.error("ERROR WHILE SENDING EMAIL FOR DUPLICATE ORDER")
              log.info("RESPONSE STATUS CODE IS : ", response.status)
            }
          }
        }
